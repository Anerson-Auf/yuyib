//! High-level glTF material override policy — fixture-free visual smoke.
//!
//! Shows the preferred game-code path:
//!
//! 1. build a declarative [`ModelMaterialPolicy`];
//! 2. attach it with [`GltfSceneLoadConfig::with_material_policy`];
//! 3. read [`LoadedGltfScene::diagnostics_summary`] after load.
//!
//! The embedded GLB has two panels that both start on washed-out `material_0`.
//! After the policy runs you should see a dark blue-gray left panel and a bright
//! red emissive right panel. Console prints importer + policy diagnostics.
//!
//! Low-level escape hatch (not used here): `model_mut_before_publication` +
//! `add_material` / `set_primitive_material`.
//!
//! ```text
...
use yuyib::{
    ...
    model::{Material, MaterialFactorPatch, ModelMaterialPolicy},
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // High-level path: declarative policy on the load config.
    let policy = ModelMaterialPolicy::new()
        .patch_named(
            "material_0",
            MaterialFactorPatch::new()
                .with_base_color_factor([0.04, 0.08, 0.16, 1.0])
                .with_metallic_roughness(0.05, 0.75)
                .with_double_sided(true),
        )
        .add_and_remap_meshes(
            Material::new()
                .with_name("yuyib.demo_red_neon")
                .with_base_color_factor([0.9, 0.02, 0.08, 1.0])
                .with_metallic_roughness(0.0, 0.35)
                .with_emissive_factor([2.6, 0.05, 0.12])
                .with_double_sided(true),
            [1],
        );
    ...
    Application::new()
        .window(WindowConfig {
            title: "Yuyib — glTF material policy (high-level)".to_owned(),
            width: 960,
            height: 540,
            ..Default::default()
        })
        ...
        .on_frame(move |context| {
            ...
            let replacement = match &mut *state {
                ...
                DemoState::Failed { message, reported } => {
                    if !*reported {
                        eprintln!("material policy demo failed: {message}");
                        *reported = true;
                    }
                    None
                }
...
impl DemoScene {
    fn new(scene: LoadedGltfScene) -> Result<Self, Box<dyn Error>> {
        ...
        if summary.is_empty() {
            println!("no import/policy diagnostics");
        } else {
        ...
        println!(
            "Expect: left panel dark blue-gray (patched material_0), right panel red neon emissive."
        );
...
/// Two side-by-side panels that both start on factor-only `material_0`.
fn write_demo_glb() -> Result<PathBuf, Box<dyn Error>> {
    ...
    let json = format!(
        r#"{{"asset":{{"version":"2.0"}},"scene":0,"scenes":[{{"nodes":[0,1]}}],"nodes":[{{"name":"left_panel","mesh":0,"translation":[-1.15,0,0]}},{{"name":"right_panel","mesh":1,"translation":[1.15,0,0]}}],"materials":[{{"name":"material_0","pbrMetallicRoughness":{{"baseColorFactor":[0.85,0.85,0.85,1],"metallicFactor":0,"roughnessFactor":1}}}}],"meshes":[{{"name":"left","primitives":[{{"attributes":{{"POSITION":1,"NORMAL":2}},"indices":0,"material":0}}]}},{{"name":"right","primitives":[{{"attributes":{{"POSITION":1,"NORMAL":2}},"indices":0,"material":0}}]}}],"buffers":[{{"byteLength":{}}}],"bufferViews":[{{"buffer":0,"byteOffset":0,"byteLength":12}},{{"buffer":0,"byteOffset":12,"byteLength":48}},{{"buffer":0,"byteOffset":60,"byteLength":48}}],"accessors":[{{"bufferView":0,"componentType":5123,"count":6,"type":"SCALAR"}},{{"bufferView":1,"componentType":5126,"count":4,"type":"VEC3","min":[-0.6,-0.6,0],"max":[0.6,0.6,0]}},{{"bufferView":2,"componentType":5126,"count":4,"type":"VEC3"}}]}}"#,
        binary.len()
    );
    let glb = pack_glb(json.as_bytes(), binary)?;
    let stamp = SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis();
    let path = std::env::temp_dir().join(format!("yuyib_material_policy_{stamp}.glb"));
    fs::write(&path, glb)?;
    Ok(path)
}
